...
try-catch는 판별하지 않는다. 그저 흘려보내다가, 오류가 발생하면 그제서야 설명서(Exception table)를 참고하여 해결방법(catch)으로 이동한다.
...
IF가 미리 판별하고 시도 자체를 안한다면\
try-catch는 일단 기능을 호출하고 거기서 throw 된 Exception을 처리한다.
